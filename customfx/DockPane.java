...
public class DockPane extends StackPane implements EventHandler<DockEvent>, Serializable {
	private static final long serialVersionUID = 6106433988621514407L;
...
//		dockAreaIndicator.getStyleClass().add("dock-area-indicator");
	...
	public final static String getDefaultUserAgentStyleheet() {
		return DockPane.class.getResource("/application.css").toExternalForm();
	}
	...
	public final static void initializeDefaultUserAgentStylesheet() {
		StyleManager.getInstance().addUserAgentStylesheet(DockPane.class.getResource("/application.css").toExternalForm());
	}
	...
	private void addNodeToLayout(Node node, DockPos dockPos, Node sibling, DockPanePosition dockPanePosition) {
		if(node instanceof DockNode) {
			List<LayoutNode> layoutList = (List<LayoutNode>)Context.getContext().get(Constants.LAYOUT_LIST);
			DockNode dockNode = (DockNode) node;
			LayoutNode nodeToBeDropped = dockNode.getLayoutNode();
			for (LayoutNode layoutNode : layoutList) {
				if((layoutNode != nodeToBeDropped) && (layoutNode.getRelativeDockNode() != null)) {
					if(nodeToBeDropped.getNodeType().equalsIgnoreCase(Constants.CHART_NODE)) {
						if(layoutNode.getRelativeDockNode().equalsIgnoreCase(Integer.toString(nodeToBeDropped.getChartUID()))) {
							layoutNode.setDockPos(nodeToBeDropped.getDockPos());
							layoutNode.setRelativeDockNode(nodeToBeDropped.getRelativeDockNode());
						}
					} else {
						if(layoutNode.getRelativeDockNode().equalsIgnoreCase(nodeToBeDropped.getNodeTitle().toString())) {
							layoutNode.setDockPos(nodeToBeDropped.getDockPos());
							layoutNode.setRelativeDockNode(nodeToBeDropped.getRelativeDockNode());
						}
					}
			...
			nodeToBeDropped.setDockPos(dockPos);
			if((sibling != null) && (sibling instanceof DockNode)) {
				DockNode siblingDockNode = (DockNode) sibling;
				if(nodeToBeDropped.getNodeType().equalsIgnoreCase(Constants.CHART_NODE)) {
					nodeToBeDropped.setRelativeDockNode(Integer.toString(siblingDockNode.getLayoutNode().getChartUID()));
				} else {
					nodeToBeDropped.setRelativeDockNode(siblingDockNode.getLayoutNode().getNodeTitle().toString());
				}
			} else {
				nodeToBeDropped.setRelativeDockNode(null);
			}
			
			if(dockPanePosition == DockPanePosition.LEFT) {
				dockNode.setDockPane((DockPane) Context.getContext().get(Constants.LEFT_PANE));
			} else if(dockPanePosition == DockPanePosition.MIDDLE) {
				dockNode.setDockPane((DockPane) Context.getContext().get(Constants.MIDDLE_PANE));
			} else {
				dockNode.setDockPane((DockPane) Context.getContext().get(Constants.RIGHT_PANE));
			}
			nodeToBeDropped.setDockPanePosition(dockPanePosition);
			layoutList.remove(nodeToBeDropped);
			layoutList.add(nodeToBeDropped);
		}
	}
	 ...
	 */
	public void dock(Node node, DockPos dockPos, Node sibling, boolean isCalledByIndicatorButton, DockPanePosition dockPanePosition) {
		if(isCalledByIndicatorButton) {
			addNodeToLayout(node, dockPos, sibling, dockPanePosition);
		}
		...
		if (dockPos == DockPos.LEFT || dockPos == DockPos.TOP) {
			...
			if (splitItems.size() > 1) {
				repositionSplitPaneDividersEqually(split);
			}
	...
	private void repositionSplitPaneDividersEqually(SplitPane split) {
		double dividerCount = split.getDividers().size();
		for(int i=1; i<=dividerCount; i++) {
			double position = i/(dividerCount+1);
			split.getDividers().get(i-1).setPosition(position);
		}
	}
	 ...
	 * @param dockPos The docking position of the node relative to the sibling.
	 */
	public void dock(Node node, DockPos dockPos, boolean isCalledByindicatorButton, DockPanePosition dockPanePosition) {
		dock(node, dockPos, root, isCalledByindicatorButton, dockPanePosition);
	}
	
	public void dock(Node node, DockPos dockPos, boolean isCalledByindicatorButton) {
		dock(node, dockPos, root, isCalledByindicatorButton, null);
	}
	...
	@Override
	public void handle(DockEvent event) {
		DockPanePosition hoveringOnPanePosition = ((DockPane) event.getSource()).getDockPanePosition();
		DockPanePosition hoveringPanePosition = ((DockPane) event.getParentPane()).getDockPanePosition();
		if(((hoveringOnPanePosition == DockPanePosition.MIDDLE) && (hoveringPanePosition == DockPanePosition.MIDDLE)) || ((hoveringOnPanePosition == DockPanePosition.RIGHT) && (hoveringPanePosition == DockPanePosition.RIGHT)) || ((hoveringOnPanePosition == DockPanePosition.LEFT) && (hoveringPanePosition == DockPanePosition.LEFT)) || ((hoveringOnPanePosition == DockPanePosition.LEFT) && (hoveringPanePosition == DockPanePosition.RIGHT)) || ((hoveringOnPanePosition == DockPanePosition.RIGHT) && (hoveringPanePosition == DockPanePosition.LEFT))) {
			if (event.getEventType() == DockEvent.DOCK_ENTER) {
			...
			if (event.getEventType() == DockEvent.DOCK_RELEASED && event.getContents() != null) {
				if (dockPosDrag != null && dockIndicatorOverlay.isShowing()) {
					...
					dockNode.dock(this, dockPosDrag, dockAreaDrag, true, hoveringOnPanePosition);
				}
			}
